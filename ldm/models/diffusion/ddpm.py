...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
                 ...
                 TrainableCameraInversion_stage_config=None,
                 SR_stage_config=None,
                 num_timesteps_cond=None,
                 cond_stage_key="image",
                 cond_stage_trainable=False,
                 concat_mode=True,
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if resize_lq:
            im_lq = F.interpolate(
                im_lq,
                size=(im_gt.size(-2), im_gt.size(-1)),
                mode='bicubic',
            )
        ...
        if len(text_cond) > z_lq.size(0):
            text_cond = text_cond[:z_lq.size(0)]
        assert len(text_cond) == z_lq.size(0)
    ...
    @torch.no_grad()
    def get_input_degradation(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, val=False, text_cond=[''], return_gt=False, resize_lq=True):
