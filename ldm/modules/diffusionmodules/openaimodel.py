...
from ldm.modules.spade import SPADE,SPADE_zero
...
def cal_fea_cossim(fea_1, fea_2, save_dir=None):
    ...
    if save_dir is None:
        save_dir_1 = r'D:\cqy\stableSR\ablation_study/cos_wt.txt'
...
# spadezero
class ResBlockDual_spadezero(TimestepBlockDual):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def __init__(
        ...
        self.spade_zero = SPADE_zero(self.out_channels, semb_channels)
    ...
    def _forward(self, x, emb, s_cond):
        ...
        if self.use_scale_shift_norm:
            ...
            h = h + emb_out
            h = self.out_layers(h)
        h = self.spade_zero(h, s_cond)  #H(B,320,64,64) s_cond:dict4:(b,256,64/32/16/8)
        return self.skip_connection(x) + h #
...
if __name__ == '__main__':
    ...
    context = torch.randn(2,77,1024).to(device)
    struct = encoderunetmodelWT(x, timesteps)

    struct1 = {
        '64': torch.zeros((b, 256, 64, 64)).to(device),  # 64的张量
        '32': torch.zeros((b, 256, 32, 32)).to(device),  # 32的张量
        '16': torch.zeros((b, 256, 16, 16)).to(device),  # 16的张量
        '8': torch.zeros((b, 256, 8, 8)).to(device)  # 8的张量
    }
